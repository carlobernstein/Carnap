...
module Carnap.Languages.PurePropositional.Logic.Belot
    ( belotSDCalc, belotSDPlusCalc, BelotSD(..), BelotSDPlus(..), parseBelotSD, parseBelotSDPlus) where
...
instance Inference BelotSD PurePropLexicon (Form Bool) where
    ...
    globalRestriction (Left ded) n NegeElim1 = Just $ 
        fitchAssumptionCheck n ded [([lneg $ phin 1], [phin 2, lneg $ phin 2])]
    globalRestriction (Left ded) n NegeElim2 = Just $ 
        fitchAssumptionCheck n ded [([lneg $ phin 1], [phin 2, lneg $ phin 2])]
    globalRestriction (Left ded) n NegeElim3 = Just $ 
        fitchAssumptionCheck n ded [([lneg $ phin 1], [phin 2, lneg $ phin 2])]
    globalRestriction (Left ded) n NegeElim4 = Just $ 
        fitchAssumptionCheck n ded [([lneg $ phin 1], [phin 2, lneg $ phin 2])]
    globalRestriction (Left ded) n DisjElim1 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 3]), ([phin 2], [phin 3])]
    globalRestriction (Left ded) n DisjElim2 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 3]), ([phin 2], [phin 3])]
    globalRestriction (Left ded) n DisjElim3 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 3]), ([phin 2], [phin 3])]
    globalRestriction (Left ded) n DisjElim4 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 3]), ([phin 2], [phin 3])]
    globalRestriction (Left ded) n BicoIntro1 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 2]), ([phin 2], [phin 1])]
    globalRestriction (Left ded) n BicoIntro2 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 2]), ([phin 2], [phin 1])]
    globalRestriction (Left ded) n BicoIntro3 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 2]), ([phin 2], [phin 1])]
    globalRestriction (Left ded) n BicoIntro4 = Just $ 
        fitchAssumptionCheck n ded [([phin 1], [phin 2]), ([phin 2], [phin 1])]
...
data BelotSDPlus = SD BelotSD | MT | HS 
                     | DS1  | DS2 
                     | Com1 | Com2 | Com3 | Com4
...
instance Show BelotSDPlus where
        show (SD x) = show x
        show MT = "MT"
        show HS = "HS"
        show DS1 = "DS"
...
instance Inference BelotSDPlus PurePropLexicon (Form Bool) where
...
parseBelotSDPlus :: RuntimeDeductionConfig PurePropLexicon (Form Bool) -> Parsec String u [BelotSDPlus]
parseBelotSDPlus rtc = try (map SD <$> parseBelotSD rtc) <|> parsePlus
    where parsePlus = do _ <- char '('
                         r <- choice (map (try . string) ["MT","HS","DS","Com","Assoc","Impl", "DN", "DeM", "Idem", "Trans", "Exp", "Dist", "Equiv"])
                         _ <- char ')'
                         return $ case r of
                                    r | r == "MT" -> [MT]
                                      | r == "HS" -> [HS]
...
parseBelotSDPlusProof :: RuntimeDeductionConfig PurePropLexicon (Form Bool) -> String -> [DeductionLine BelotSDPlus PurePropLexicon (Form Bool)]
parseBelotSDPlusProof ders = toDeductionFitchAlt (parseBelotSDPlus ders) (purePropFormulaParser belotOpts)
